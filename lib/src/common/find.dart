...
DriverError _createInvalidKeyValueTypeError(String invalidType) {
  return DriverError(
      'Unsupported key value type $invalidType. Flutter Driver only supports ${_supportedKeyValueTypes
          .join(", ")}');
}
...
abstract class CommandWithTarget extends Command {
  ...
  CommandWithTarget(this.finder, {Duration timeout}) : super(timeout: timeout) {
    if (finder == null) throw DriverError('$runtimeType target cannot be null');
  }
...
class WaitUntilNoTransientCallbacks extends Command {
  ...
  const WaitUntilNoTransientCallbacks({Duration timeout})
...
abstract class SerializableFinder {
  ...
  static SerializableFinder deserialize(Map<String, String> json) {
    ...
    switch (finderType) {
      case 'ByIndex':
        return ByIndex.deserialize(json);
      case 'ByType':
      ...
      case 'Descendant':
        return Descendant.deserialize(json);
      case 'Ancestor':
        return Ancestor.deserialize(json);
    }
    throw DriverError('Unsupported search specification type $finderType');
  }
}

class ByIndex extends SerializableFinder {
  /// Creates a tooltip finder given the tooltip's message [text].
  const ByIndex(this.finder,
      this.index,);
  ...
  Map<String, String> serialize() =>
      super.serialize()
        ..addAll(<String, String>{
          ...
          'finder': jsonEncode(finder.serialize())
        });

  /// Deserializes the finder from JSON generated by [serialize].
  static ByIndex deserialize(Map<String, String> json) {
    return ByIndex(
      SerializableFinder.deserialize(
          Map.from(jsonDecode(json['finder']))),
...
class Descendant extends SerializableFinder {
  ...
  Map<String, String> serialize() {
    return super.serialize()
      ..addAll(of.serialize().map((String key, String value) =>
          MapEntry<String, String>('of_$key', value)))..addAll(
          matching.serialize().map((String key, String value) =>
              MapEntry<String, String>('matching_$key', value)))..addAll(
          <String, String>{
