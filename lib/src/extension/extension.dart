...
class _DriverBinding extends BindingBase
  ...
  void initServiceExtensions() {
    ...
    final FlutterDriverExtension extension =
        FlutterDriverExtension(_handler, _silenceErrors);
...
void enableFlutterDriverExtension(
    {DataHandler handler, bool silenceErrors = false}) {
...
class FlutterDriverExtension {
  ...
  FlutterDriverExtension(this._requestDataHandler, this._silenceErrors) {
    ...
    _commandDeserializers.addAll(<String, CommandDeserializerCallback>{
      'get_health': (Map<String, String> params) =>
          GetHealth.deserialize(params),
      'get_render_tree': (Map<String, String> params) =>
          GetRenderTree.deserialize(params),
      'enter_text': (Map<String, String> params) =>
          EnterText.deserialize(params),
      'get_text': (Map<String, String> params) => GetText.deserialize(params),
      'request_data': (Map<String, String> params) =>
          RequestData.deserialize(params),
      'scroll': (Map<String, String> params) => Scroll.deserialize(params),
      'scrollIntoView': (Map<String, String> params) =>
          ScrollIntoView.deserialize(params),
      'set_frame_sync': (Map<String, String> params) =>
          SetFrameSync.deserialize(params),
      'set_semantics': (Map<String, String> params) =>
          SetSemantics.deserialize(params),
      ...
      'waitUntilNoTransientCallbacks': (Map<String, String> params) =>
          WaitUntilNoTransientCallbacks.deserialize(params),
      'get_semantics_id': (Map<String, String> params) =>
          GetSemanticsId.deserialize(params),
      'get_offset': (Map<String, String> params) =>
          GetOffset.deserialize(params),
      'get_diagnostics_tree': (Map<String, String> params) =>
          GetDiagnosticsTree.deserialize(params),
    });
    ...
    _finders.addAll(<String, FinderConstructor>{
      ...
      'Descendant': (SerializableFinder finder) =>
          _createDescendantFinder(finder),
    });
  }
  ...
  final Map<String, CommandHandlerCallback> _commandHandlers =
      <String, CommandHandlerCallback>{};
  final Map<String, CommandDeserializerCallback> _commandDeserializers =
  ...
  Future<Map<String, dynamic>> call(Map<String, String> params) async {
    ...
    try {
      final CommandHandlerCallback commandHandler =
          _commandHandlers[commandKind];
      ...
      if (!_silenceErrors) _log.error(msg);
      return _makeResponse(msg, isError: true);
    }
  }
  ...
  Finder _createByTooltipMessageFinder(ByTooltipMessage arguments) {
    return find.byElementPredicate((Element element) {
      ...
      if (widget is Tooltip) return widget.message == arguments.text;
      return false;
  ...
  Future<GetSemanticsIdResult> _getSemanticsId(Command command) async {
    ...
    final Finder target =
        await _waitForElement(_createFinder(semanticsCommand.finder));
  ...
  Future<GetOffsetResult> _getOffset(Command command) async {
    ...
    final Finder finder =
        await _waitForElement(_createFinder(getOffsetCommand.finder));
  ...
  Future<DiagnosticsTreeResult> _getDiagnosticsTree(Command command) async {
    ...
    final Finder finder =
        await _waitForElement(_createFinder(diagnosticsCommand.finder));
    final Element element = finder.evaluate().single;
  ...
  Future<ScrollResult> _scroll(Command command) async {
    ...
    final Finder target =
        await _waitForElement(_createFinder(scrollCommand.finder));
  ...
  Future<ScrollResult> _scrollIntoView(Command command) async {
    ...
    final Finder target =
        await _waitForElement(_createFinder(scrollIntoViewCommand.finder));
    await Scrollable.ensureVisible(target.evaluate().single,
        duration: const Duration(milliseconds: 100),
        alignment: scrollIntoViewCommand.alignment ?? 0.0);
    return const ScrollResult();
  }
  ...
  Future<GetTextResult> _getText(Command command) async {
    ...
    final Finder target =
        await _waitForElement(_createFinder(getTextCommand.finder));
    // TODO(yjbanov): support more ways to read text
    final Text text = target.evaluate().single.widget;
    return GetTextResult(text.data);
  }

  Future<SetTextEntryEmulationResult> _setTextEntryEmulation(
